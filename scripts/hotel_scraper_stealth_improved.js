#!/usr/bin/env node
/**
 * Improved Stealth Hotel Scraper for Expedia using Puppeteer
 * Enhanced data extraction with better hotel name parsing
 */

const puppeteer = require('puppeteer');
const fs = require('fs');

class ImprovedStealthExpediaScraper {
    constructor(options = {}) {
    ...
    async scrapeHotels(location, maxHotels = 10, headless = true, useBrightData = true) {
        ...
        try {
            console.log(`🕵️ Improved stealth scraping ${maxHotels} hotels from Expedia for: ${location}`);
            ...
            const hotels = await this.extractHotelDataImproved(page, maxHotels);
            ...
            console.log(`✅ Successfully scraped ${hotels.length} hotels with improved stealth mode`);
            return hotels;

        } catch (error) {
            console.error('❌ Error during improved stealth scraping:', error.message);
            return this.generateFallbackHotels(location, maxHotels);
        } finally {
            if (browser) {
                try {
    ...
    async stealthNavigate(page, searchUrl, useBrightData) {
        try {
            ...
            const hotelElements = await page.$$('[data-testid*="property"], [data-testid*="hotel"], .uitk-layout-flex, h3.uitk-heading, [data-testid*="uitk-gallery-item"], [class*="property-card"], [class*="hotel-card"]');
    ...
    async extractHotelDataImproved(page, maxHotels) {
        try {
            console.log('🔍 Extracting hotel data with improved parsing...');
            
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            let hotelCards = [];
            
            // Try multiple approaches to find hotel cards
            hotelCards = await page.$$('[data-testid*="property-card"], [data-testid*="hotel-card"], [data-testid*="uitk-gallery-item"], [class*="property-card"], [class*="hotel-card"]');
            console.log(`🏨 Found ${hotelCards.length} hotel cards with specific selectors`);
            
            if (hotelCards.length === 0) {
                hotelCards = await page.$$('.uitk-layout-flex, [class*="uitk-card"], [class*="hotel"], [class*="property"]');
                console.log(`🏨 Found ${hotelCards.length} potential hotel elements with general selectors`);
            }
            
            if (hotelCards.length === 0) {
                const allElements = await page.$$('div, section, article');
                console.log(`🏨 Found ${allElements.length} total elements, filtering for hotel content...`);
                
                for (const element of allElements) {
                    try {
                        const text = await element.evaluate(el => el.textContent?.trim() || '');
                        if (text.length > 10 && text.length < 500 && 
                            (text.includes('Hotel') || text.includes('Inn') || text.includes('Resort') || 
                             text.includes('$') || text.includes('night') || text.includes('reviews'))) {
                            hotelCards.push(element);
                        }
                    } catch (error) {
                        continue;
                    }
                }
                console.log(`🏨 Filtered to ${hotelCards.length} potential hotel elements`);
            }
            ...
            for (let i = 0; i < limit; i++) {
                try {
                    const hotelData = await this.extractSingleHotelImproved(hotelCards[i]);
                    if (hotelData && hotelData.name && hotelData.name !== 'Unknown Hotel' && 
                        !hotelData.name.includes('Dates') && !hotelData.name.includes('Room')) {
            ...
            console.log(`🎉 Successfully extracted ${hotels.length} hotels with improved parsing`);
            return hotels;
    ...
    async extractSingleHotelImproved(hotelCard) {
        try {
            let name = await this.extractHotelNameImproved(hotelCard);
            
            if (!name || name.length < 3 || 
                name.includes('Dates') || name.includes('Room') || 
                name.includes('Aug') || name.includes('night') ||
                name.includes('$') || name.includes('reviews')) {
                return null;
            }

            const price = await this.extractPriceImproved(hotelCard);
            const rating = await this.extractRatingImproved(hotelCard);
            const images = await this.extractHotelImagesImproved(hotelCard);
            const reviews = await this.extractReviewsImproved(hotelCard);

            return {
                id: `improved_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
                name: name,
                price: price,
                currency: 'USD',
                rating: rating,
                ...
                images: images,
                bookingUrl: `https://www.expedia.com/Hotel-Search?destination=${encodeURIComponent(name)}`,
                address: 'Address not available',
                location: { lat: 0, lng: 0 },
                amenities: ['WiFi', 'Air Conditioning', 'Free Breakfast'],
                description: `Comfortable accommodation - ${name}`,
                scrapedAt: new Date().toISOString(),
                source: 'Expedia (Improved Stealth)'
            };
    ...
    async extractHotelNameImproved(hotelCard) {
        try {
            const nameSelectors = [
                'h3.uitk-heading',
                'h2.uitk-heading',
            ...
            for (const selector of nameSelectors) {
                try {
                    const nameElement = await hotelCard.$(selector);
                    if (nameElement) {
            ...
            try {
                ...
                for (const line of lines) {
                    ...
                    if (trimmedLine.length > 5 && 
                        ...
                        !trimmedLine.toLowerCase().includes('dates') &&
                        !trimmedLine.toLowerCase().includes('room') &&
                        !trimmedLine.toLowerCase().includes('aug') &&
                        (trimmedLine.includes('Hotel') || trimmedLine.includes('Inn') || 
                         trimmedLine.includes('Resort') || trimmedLine.includes('Plaza') ||
                         trimmedLine.includes('Marriott') || trimmedLine.includes('Hilton') ||
                         trimmedLine.includes('Hyatt') || trimmedLine.includes('Sheraton'))) {
                        console.log(`🔍 Extracted hotel name from text: ${trimmedLine}`);
                        return trimmedLine;
                    }
                }
            } catch (error) {
            ...
            console.error('❌ Error extracting hotel name:', error.message);
            return null;
        }
    }

    async extractPriceImproved(hotelCard) {
        try {
            ...
            for (const selector of priceSelectors) {
                try {
                    const priceElement = await hotelCard.$(selector);
                    if (priceElement) {
    ...
    async extractRatingImproved(hotelCard) {
        try {
            ...
            for (const selector of ratingSelectors) {
                try {
                    const ratingElement = await hotelCard.$(selector);
                    if (ratingElement) {
                        ...
                        if (extractedRating > 0) {
                            console.log(`⭐ Found rating: ${extractedRating}`);
    ...
    async extractReviewsImproved(hotelCard) {
        try {
            ...
            for (const selector of reviewSelectors) {
                try {
                    const reviewElement = await hotelCard.$(selector);
                    if (reviewElement) {
                        const reviewText = await reviewElement.evaluate(el => el.textContent?.trim());
                        const reviewCount = this.extractReviewCountFromText(reviewText);
                        if (reviewCount > 0) {
    ...
    async extractHotelImagesImproved(hotelCard) {
        try {
    ...
    extractReviewCountFromText(reviewText) {
        try {
            ...
            console.error('❌ Error extracting review count:', error.message);
        }
        return 0;
    }
...
async function main() {
    ...
    if (args.length < 1) {
        console.log('Usage: node hotel_scraper_stealth_improved.js <location> [max_hotels] [headless] [use_bright_data]');
        console.log('Example: node hotel_scraper_stealth_improved.js "New York" 5 false true');
        process.exit(1);
    }
    ...
    console.log(`🕵️ Starting improved stealth hotel scraper for: ${location}`);
    console.log(`Headless mode: ${headless}`);
    console.log(`Using Bright Data: ${useBrightData}`);

    const scraper = new ImprovedStealthExpediaScraper({
        headless: headless,
    ...
    try {
        ...
        hotels.forEach((hotel, i) => {
            ...
            console.log(`   📝 Reviews: ${hotel.reviewCount} (${hotel.avgReview})`);
            console.log(`   📍 Source: ${hotel.source}`);
            console.log();
        });

        // Save to file
        const outputFile = `improved_stealth_expedia_hotels_${location.replace(/[^a-zA-Z0-9]/g, '_')}.json`;
...
module.exports = ImprovedStealthExpediaScraper;
