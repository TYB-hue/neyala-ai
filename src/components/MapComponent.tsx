...
const MapComponent: React.FC<MapProps> = ({ center, places }) => {
  ...
  const updateMarkers = useCallback((map: any) => {
    ...
    places.forEach((place) => {
      ...
      // Get photo URL from itinerary card image (thumbnail)
      // This should match the image displayed in the AttractionCard
      ...
      // Use the itinerary card thumbnail image (photoUrl from itinerary data)
      // This is the same image shown in the AttractionCard component
      let photoUrl = place.photoUrl;
      
      // If no photoUrl, use placeholder
      ...
      const infoWindow = new window.google.maps.InfoWindow({
        content: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; width: 260px; padding: 0; margin: 0; overflow: hidden; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <!-- Large Image Section - Shows itinerary card thumbnail -->
            <img src="${photoUrl}" 
                 style="width: 100%; height: 160px; object-fit: cover; display: block; margin: 0; padding: 0; background-color: #f3f4f6;" 
                 alt="${place.name}"
                 loading="lazy"
