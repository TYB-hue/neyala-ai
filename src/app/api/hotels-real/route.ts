...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Using real Booking.com scraper...');

    try {
      // Use the real Booking.com scraper
      ...
      console.log(`Scraper returned ${hotels.length} hotels`);
      ...
      if (hotels.length > 0) {
        ...
        console.log(`Data source: ${isRealData ? 'Real Booking.com' : 'Fallback'}`);
        ...
        return NextResponse.json({
          ...
          source: isRealData ? 'Booking.com (Real Data)' : 'Fallback Data'
        });
      } else {
        console.log('No hotels returned from scraper, using fallback');
        return await generateFallbackHotels(destination);
      }
    } catch (scraperError) {
